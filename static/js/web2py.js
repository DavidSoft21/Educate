...
(function ($, undefined) {
  ...
  const FORMDATA_IS_SUPPORTED = typeof (FormData) !== 'undefined'
  ...
  $.web2py = web2py = {
    ...
    popup: function (url) {
      /* popup a window */
      const newwindow = window.open(url, 'name', 'height=400,width=600')
    ...
    ajax: function (u, s, t, options) {
      ...
      const success_function = function (msg) {
        if (t) {
          if (t == ':eval') eval(msg)
      ...
      $('ul.w2p_list', target).each(function () {
        ...
        function rel (ul) {
          ...
          $(ul).find('li').each(function () {
            const trimmed = $.trim($(this.firstChild).val())
    ...
    after_ajax: function (xhr) {
      ...
      const command = xhr.getResponseHeader('web2py-component-command')
      const flash = xhr.getResponseHeader('web2py-component-flash')
      if (command !== null) {
    ...
    event_handlers: function () {
      ...
      doc.on('keyup', 'input.integer', function () {
        const nvalue = this.value.reverse().replace(/[^0-9\-]|\-(?=.)/g, '').reverse()
        if (this.value != nvalue) this.value = nvalue
      })
      doc.on('keyup', 'input.double, input.decimal', function () {
        const nvalue = this.value.reverse().replace(
          /[^0-9\-\.,]|[\-](?=.)|[\.,](?=[0-9]*[\.,])/g, '').reverse()
        if (this.value != nvalue) this.value = nvalue
      })
      const confirm_message = !web2py.isUndefined(w2p_ajax_confirm_message)
        ? w2p_ajax_confirm_message
        : 'Are you sure you want to delete this object?'
      doc.on('click', 'input[type="checkbox"].delete', function () {
        if (this.checked) { if (!web2py.confirm(confirm_message)) this.checked = false }
      })
      const datetime_format = !web2py.isUndefined(w2p_ajax_datetime_format)
        ? w2p_ajax_datetime_format
        : '%Y-%m-%d %H:%M:%S'
      doc.on('click', 'input.datetime', function () {
        const tformat = $(this).data('w2p_datetime_format')
        const active = $(this).data('w2p_datetime')
        const format = !web2py.isUndefined(tformat) ? tformat : datetime_format
        if (active === undefined) {
          Calendar.setup({
            inputField: this,
            ifFormat: format,
      ...
      const date_format = !web2py.isUndefined(w2p_ajax_date_format) ? w2p_ajax_date_format : '%Y-%m-%d'
      doc.on('click', 'input.date', function () {
        const tformat = $(this).data('w2p_date_format')
        const active = $(this).data('w2p_date')
        const format = !web2py.isUndefined(tformat) ? tformat : date_format
        if (active === undefined) {
          Calendar.setup({
            inputField: this,
            ifFormat: format,
      ...
      doc.on('focus', 'input.time', function () {
        const active = $(this).data('w2p_time')
      ...
      $(doc).on('submit', 'form', function (e) {
        const submit_buttons = $(this).find(web2py.formInputClickSelector)
      ...
      doc.ajaxSuccess(function (e, xhr) {
        const redirect = xhr.getResponseHeader('web2py-redirect-location')
        if (redirect !== null) {
    ...
    trap_form: function (action, target) {
      ...
      $('#' + target + ' form').each(function () {
        ...
        const w2p_target = $(this).attr('data-w2p_target')
    ...
    ajax_page: function (method, action, data, target, element) {
      ...
      const contentType = isFormData ? false : 'application/x-www-form-urlencoded; charset=UTF-8'
    ...
    component: function (action, target, timeout, times, el) {
      ...
      $(function () {
        const jelement = $('#' + target)
        const element = jelement.get(0)
        const statement = 'jQuery("#' + target + '").get(0).reload();'
    ...
    updatePage: function (xhr, target) {
      const t = $('#' + target)
      const html = $.parseHTML(xhr.responseText, document, true)
      const title_elements = $(html).filter('title').add($(html).find('title'))
      const title = title_elements.last().text()
      if (title) {
        title_elements.remove() /* Remove any title elements from the response */
        document.title = $.trim(title) /* Set the new document title */
      }
      const content = xhr.getResponseHeader('web2py-component-content')
    ...
    calc_entropy: function (mystring) {
      ...
      for (let i = 0; i < mystringlist.length; i++) { /* classify this character */
        const c = mystringlist[i]
        let inset = 5
      ...
      const entropy = mystring.length * Math.log(score) / 0.6931471805599453
      return Math.round(entropy * 100) / 100
    },
    validate_entropy: function (myfield, req_entropy) {
      if (!web2py.isUndefined(myfield.data('w2p_entropy'))) req_entropy = myfield.data('w2p_entropy')
      const validator = function () {
        const v = (web2py.calc_entropy(myfield.val()) || 0) / req_entropy
        let r = 0
        let g = 0
        const b = 0
        const rs = function (x) {
        ...
        const color = '#' + rs(r) + rs(g) + rs(b)
        myfield.css('background-color', color)
        const entropy_callback = myfield.data('entropy_callback')
        if (entropy_callback) entropy_callback(v)
      }
      if (!myfield.hasClass('entropy_check')) {
        myfield.on('keyup', validator).on('keydown', validator)
    ...
    disableElement: function (el) {
      ...
      const method = el.is('input') ? 'val' : 'html'
      // method = el.attr('name') ? 'html' : 'val';
      const disable_with_message = (!web2py.isUndefined(w2p_ajax_disable_with_message))
    ...
    show_if_handler: function (target) {
      const triggers = {}
      const show_if = function () {
        const t = $(this)
        const id = t.attr('id')
        t.attr('value', t.val())
        for (let k = 0; k < triggers[id].length; k++) {
          const dep = $('#' + triggers[id][k], target)
          const tr = $('#' + triggers[id][k] + '__row', target)
          if (t.is(dep.attr('data-show-if'))) tr[animateIn]()
          else tr.hide()
        }
      }
      $('[data-show-trigger]', target).each(function () {
        const name = $(this).attr('data-show-trigger')
    ...
    component_handler: function (target) {
      $('div[data-w2p_remote]', target).each(function () {
        let remote, times, timeout, target
        const el = $(this)
    ...
    a_handler: function (el, e) {
      e.preventDefault()
      const method = el.data('w2p_method')
      const action = el.attr('href')
      const target = el.data('w2p_target')
      let confirm_message = el.data('w2p_confirm')

      const pre_call = el.data('w2p_pre_call')
    ...
    a_handlers: function () {
      ...
      el.on('ajax:success', 'a[data-w2p_method][data-w2p_remove]', function () {
        const el = $(this)
        let toremove = el.data('w2p_remove')
    ...
    disableFormElements: function (form) {
      form.find(web2py.disableSelector).each(function () {
        ...
        const disable_with = element.data('w2p_disable_with')
        const disable = element.data('w2p_disable')
